...
test("real Node/Pi loader and agent loop bind captures, project packets, invalidate writes and honor off", () => {
  ...
  try {
    ...
    expect(JSON.parse(result.stdout)).toEqual({ turns: 3, captureBound: true, packetInvalidated: true, errors: [] });
