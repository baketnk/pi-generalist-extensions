...
export default function subagents(pi: ExtensionAPI, options: { workerEntry?: string; home?: string } = {}) {
    ...
    async execute(callId, params, signal, _update, ctx) {
      ...
      switch (params.action) {
        case "start": {
          ...
          const launchEpoch = branchEpoch, agentDir = getAgentDir();
          const model = resolveWorkerModel(params.model, ctx.model, () => loadModelConfig(agentDir));
        ...
        case "models": {
          const config = loadModelConfig(getAgentDir());
          result = { parent: ctx.model ? { provider: ctx.model.provider, id: ctx.model.id } : null,
  ...
  pi.registerCommand("subagents", { description: "Worker view; stop cancels; ladder [provider/model ...] configures next-smaller routing.", async handler(args, ctx) {
    ...
    if (args.trim()) throw new Error("Use /subagents, /subagents stop, or /subagents ladder [provider/model ...].");
