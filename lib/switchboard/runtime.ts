...
import { BoardClient, bindingAt, ensureService, type Binding } from "./client.ts";
...
export interface RuntimeOptions {
  ...
  onChange?: () => void; ensure?: typeof ensureService; intervalMs?: number;
}
export class BoardRuntime {
  ...
  private async initialize() {
    try {
      ...
      this.inputCard = { ...project, name: clipped(plain(this.options.name || `unnamed · ${this.options.sessionId.slice(0, 8)}`), 160), summary: "", activity: this.earlyActivity };
      this.life.signal.throwIfAborted();
      await this.sync();
  ...
  private changed() { if (!this.closed) this.options.onChange?.(); }
  ...
  sync(): Promise<void> {
    const next = this.syncTail.then(async () => {
      ...
      try {
        ...
        this.state = "online"; this.error = undefined;
        if (!this.watching) {
  ...
  update(input: { name?: string; summary?: string; activity?: Activity }) {
    ...
    if (input.name !== undefined) this.inputCard.name = clipped(plain(input.name || `unnamed · ${this.options.sessionId.slice(0, 8)}`), 160);
