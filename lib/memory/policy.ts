...
export interface MemoryPolicy {
  ...
  profile: "default" | "project" | "continuity"; personalId?: string; configDigest?: string;
}
export function readMemoryPolicy(ctx: ExtensionContext): MemoryPolicy {
  const sessionId = ctx.sessionManager.getSessionId();
  let state: MemoryPolicy = { version: 1, sessionId, cwd: ctx.cwd, enabled: false, profile: "default" };
  for (const entry of ctx.sessionManager.getBranch()) {
    if (entry.type !== "custom" || entry.customType !== POLICY_ENTRY) continue;
    const data = entry.data as MemoryPolicy | undefined;
    state = { version: 1, sessionId, cwd: ctx.cwd, enabled: false, profile: "default" };
    if (!data || data.sessionId !== sessionId || data.cwd !== ctx.cwd) continue; // forks/new cwd never inherit activation
    try {
      if (data.version !== 1 || typeof data.enabled !== "boolean" || !isAbsolute(data.cwd) || !["default", "project", "continuity"].includes(data.profile)) continue;
